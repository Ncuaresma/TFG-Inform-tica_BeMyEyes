...
/* // Lineas totalmente horizontales y verticales
...
*/
   // Lineas semi horizontales y verticales
    for (int i = 0; i < lines.size(); i++) {
        //Print the lines on a file
        //print only the lines that are horizontal(Compare if y1 == y2)
 ...
 /*   // draw the result as big green lines:
    ...
    {
        cv::line(input, cv::Point(vertical_lines[i][2], vertical_lines[i][3]), cv::Point(vertical_lines[i][4], vertical_lines[i][5]), cv::Scalar(0, 255, 0), 5);
    }
